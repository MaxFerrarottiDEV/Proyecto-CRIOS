...
from django import forms  # type: ignore
from .models import MesasExamenes, MateriasxplanesEstudios
from .models import MateriasxplanesEstudios
...
class PreinscripcionForm(forms.ModelForm):
    ...
    fecha_nac = forms.CharField(label='Fecha de Nacimiento:',
                                required=True, widget=forms.DateInput(attrs={'type': 'date'}),)
    provincia = forms.ChoiceField(
        label='Provincia:', choices=PROVINCIA_CHOICES, required=True,)
    dni = forms.CharField(label='DNI (Solo numeros):',
                          max_length=10, required=True,)
    edad = forms.CharField(label='Edad:', max_length=2, required=True)
    domicilio = forms.CharField(
        label='Domicilio:', max_length=200, required=True)
    telefono_fijo = forms.CharField(
        label='Teléfono Fijo:', max_length=12, required=False,)
    celular_nro = forms.CharField(
        label='Número de Celular:', max_length=12, required=True)
    ...
    estado_civil = forms.ChoiceField(label='Estado Civil:', choices=ESTADO_CIVIL_CHOICES, widget=forms.RadioSelect(
        attrs={'class': 'radio', 'style': 'font-size: 16px; margin-right: 10px;'}), required=True,)
    hijos = forms.ChoiceField(label='¿Tiene hijos?:', choices=TIENE_HIJOS_CHOICES, widget=forms.RadioSelect(
        attrs={'class': 'radio', 'style': 'font-size: 16px; margin-right: 10px;'}), required=False,)
    lugar_trabajo = forms.CharField(
        label='Lugar de Trabajo:', max_length=200, required=False,)
    tel_emergencia = forms.CharField(
        label='Teléfono de Emergencia:', max_length=12, required=True,)
    col_egreso = forms.CharField(
        label='Colegio que Egresó:', max_length=200, required=True,)
    titulo = forms.CharField(label='Título:', max_length=200, required=True,)
    otro_titulo = forms.CharField(
        label='Otros Títulos (Separar con coma):', max_length=200, required=False,)
    anio_egreso = forms.CharField(
        label='Año de Egreso:', max_length=4, required=True)
    preg_1 = forms.ChoiceField(label='¿Tuvo otro ingreso a nivel Superior o universitario anterior a este?:',
                               choices=PREG_1_CHOICES, widget=forms.RadioSelect, required=False,)
    resp_1 = forms.CharField(label='¿Cuál?', required=False,)
    resp_2 = forms.ChoiceField(label='¿Completo esos estudios?:', choices=RESP_2_CHOICES, widget=forms.RadioSelect(
        attrs={'class': 'radio', 'style': 'font-size: 16px; margin-right: 10px;'}), required=False,)
    preg_2 = forms.CharField(
        label='¿Es beneficiario de algún tipo de Beca/Ayuda Social? (Especifique):', required=False,)
    ...
    class Meta:
        ...
        # Lista de campos del modelo a excluir del formulario
        exclude = ['matricula', 'legajo_fisico']
...
class MesaExamenForm(forms.ModelForm):
    Id_MatXPlan_ME = forms.ModelChoiceField(
        queryset=MateriasxplanesEstudios.objects.select_related('id_materia'),
        label="Materia",
    ...
    Fecha_Examen = forms.DateField(
        widget=forms.DateInput(
            ...
            format='%Y-%m-%d',  # Formato esperado por el input
        ),
        input_formats=['%Y-%m-%d'],  # Acepta este formato
